...
class MobileNeedsPage extends StatefulWidget {
  const MobileNeedsPage({super.key});

  @override
  State<StatefulWidget> createState() => _MobileNeedsPageState();
}

class _MobileNeedsPageState extends State<MobileNeedsPage> {
  ...
  Future<void> _handleInkWellTap(BuildContext context, int index) async {
    switch (index) {
      ...
      case 3:
        var response =
            await sendMessage(2, "I need to go to the bathroom", null);
        if (response.statusCode == 201) {
          showOverlay(context);
        }
        break;
      case 4:
        _showBedPosDialog(context, (String selectedPosition) async {
          var response = await sendMessage(
              2,
              "I would like to change the position to: $selectedPosition",
              null);
          if (response.statusCode == 201) {
            showOverlay(context);
          }
      ...
      case 5:
        var response =
            await sendMessage(2, "I would like to contact my family", null);
        if (response.statusCode == 201) {
          showOverlay(context);
        }
        break;
