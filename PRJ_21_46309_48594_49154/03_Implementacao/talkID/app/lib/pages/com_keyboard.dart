...
class ComKeyboardPage extends StatefulWidget {
  const ComKeyboardPage({super.key});

  @override
  State<StatefulWidget> createState() => _ComKeyboardPageState();
...
class _ComKeyboardPageState extends State<ComKeyboardPage> {
  ...
  Widget build(BuildContext context) {
    ...
    //print("country code: ${provider.locale.countryCode}");
    return Scaffold(
      ...
      return FutureBuilder<dynamic>(
          ...
          builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
            if (snapshot.hasData) {
              ...
              return Text(
                  AppLocalizations.of(context)?.errorLoadingEngines ?? '');
            } else {
              return Text(AppLocalizations.of(context)?.loadingEngines ?? '');
            }
          });
    } else {
  ...
  Widget _futureBuilder() => FutureBuilder<dynamic>(
      ...
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          ...
          return Text(
              AppLocalizations.of(context)?.errorLoadingLanguages ?? '');
        } else {
          return Text(AppLocalizations.of(context)?.loadingLanguages ?? '');
        }
      });
  ...
  Widget _languageDropDownSection(List<dynamic> languages) => Container(
        ...
        Visibility(
          visible: isAndroid,
          child: Text(
              "${AppLocalizations.of(context)?.isInstalled ?? ': '} $isCurrentLanguageInstalled"),
        ),
      ]));
  ...
  Widget _getMaxSpeechInputLengthSection() {
    return Row(
      ...
      children: [
        ElevatedButton(
          child: Text(AppLocalizations.of(context)?.speechInputLength ?? ''),
          onPressed: () async {
        ...
        Text(
            "$_inputLength ${AppLocalizations.of(context)?.speechInputLength ?? ''}"),
      ],
    );
  }
  ...
  Widget _buildSliders(TextTheme _textTheme) {
    return Column(
      ...
      /*
      children: [
        const Text("Volume", style: TextStyle(fontSize: 16.0)),
        _volume(), _pitch(), _rate()],
        */
    );
  }
